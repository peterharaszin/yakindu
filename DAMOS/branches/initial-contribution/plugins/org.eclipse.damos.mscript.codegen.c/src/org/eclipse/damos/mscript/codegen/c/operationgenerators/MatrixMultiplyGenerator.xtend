...
class MatrixMultiplyGenerator implements IOperationGenerator {
	...
	override generate(IMscriptGeneratorContext context, Type resultDataType, CharSequence target, Expression expression) {
		...
		val leftMatrixType = MachineDataTypes::create(context.configuration, context.getFunctionInfo.getValue(binaryExpression.leftOperand).dataType as ArrayType);
		val rightMatrixType = MachineDataTypes::create(context.configuration, context.getFunctionInfo.getValue(binaryExpression.rightOperand).dataType as ArrayType);
