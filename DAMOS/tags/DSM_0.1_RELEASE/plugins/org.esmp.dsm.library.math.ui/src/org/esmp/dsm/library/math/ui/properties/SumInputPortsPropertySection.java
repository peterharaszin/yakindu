...
import org.esmp.dsm.library.common.util.BlockConstants;
		...
		if (block != null) {
			...
			if (input != null) {
				...
				DestroyElementRequest destroyRequest = new DestroyElementRequest(
		                ...
		                false);
		        ICommand command = ElementTypes.INPUT.getEditCommand(destroyRequest);
		...
		if (block != null) {
			...
			if (input != null) {
				...
				if (index < inputPorts.size()) {
					...
					Parameter signParameter = inputPort.getParameter(SumConstants.PORT_PARAMETER__SIGN);
					if (signParameter != null) {
						double sign = BlockDiagramUtil.getParameterValueAsDouble(inputPort, SumConstants.PORT_PARAMETER__SIGN, 1);
						SetRequest setRequest = new SetRequest(
								editPart.getEditingDomain(),
								signParameter,
								BlockDiagramPackage.eINSTANCE.getParameter_Value(),
								sign >= 0 ? "-1" : "1");
				        ICommand command = ElementTypes.PARAMETER.getEditCommand(setRequest);
