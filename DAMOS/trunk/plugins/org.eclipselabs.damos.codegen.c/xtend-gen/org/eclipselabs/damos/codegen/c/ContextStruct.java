...
import org.eclipselabs.damos.codegen.c.TaskMessageStruct;
...
public class ContextStruct extends PrimaryCodeFragment {
  ...
  @Inject
  public ContextStruct(final ITaskGenerator taskGenerator) {
  ...
  public void doInitialize(final IGeneratorContext context, final IProgressMonitor monitor) {
    ...
    if (_not) {
      TaskMessageStruct _taskMessageStruct = new TaskMessageStruct();
      context.addCodeFragment(_taskMessageStruct, monitor);
    }
    Configuration _configuration = context.getConfiguration();
    final String prefix = InternalGeneratorUtil.getPrefix(_configuration);
    ...
    _builder.newLineIfNotEmpty();
    {
      for(final ComponentNode node_2 : nodes) {
        ...
        String _prefix = InternalGeneratorUtil.getPrefix(_configuration_1, node_2);
  ...
  private CharSequence getContextTypeName(final IGeneratorContext context, final ComponentNode node) {
    ...
    if (_equals) {
      Configuration _configuration = context.getConfiguration();
      String _prefix = InternalGeneratorUtil.getPrefix(_configuration, node);
