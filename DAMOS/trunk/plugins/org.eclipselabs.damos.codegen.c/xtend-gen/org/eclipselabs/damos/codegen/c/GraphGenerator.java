...
public class GraphGenerator implements IGraphGenerator {
  ...
  private boolean contributesComputeOutputsCode(final IGeneratorContext context, final Node node) {
    ...
    if ((node instanceof ComponentNode)) {
      ...
      boolean _or = false;
      boolean _or_1 = false;
      IComponentGenerator _componentGenerator = GeneratorNodeExtensions.getComponentGenerator(componentNode);
  ...
  public boolean contributesOutputVariableDeclarations(final IGeneratorContext context, final Graph graph) {
    Iterable<Node> _allNodes = graph.getAllNodes();
    Iterable<ComponentNode> _filter = Iterables.<ComponentNode>filter(_allNodes, ComponentNode.class);
    final Function1<ComponentNode,Boolean> _function = new Function1<ComponentNode,Boolean>() {
        public Boolean apply(final ComponentNode n) {
    ...
    final Function1<ComponentNode,Boolean> _function_1 = new Function1<ComponentNode,Boolean>() {
        public Boolean apply(final ComponentNode n) {
          Iterable<OutputPort> _outputVariableDeclarationPorts = GraphGenerator.this.getOutputVariableDeclarationPorts(n);
          boolean _isEmpty = IterableExtensions.isEmpty(_outputVariableDeclarationPorts);
          boolean _not = (!_isEmpty);
          return Boolean.valueOf(_not);
        }
      };
    return IterableExtensions.<ComponentNode>exists(_filter_1, _function_1);
  }
    ...
    CharSequence _xblockexpression = null;
    {
      ...
      StringConcatenation _builder = new StringConcatenation();
      {
        for(final ComponentNode node : nodes) {
          ...
          _builder.newLineIfNotEmpty();
          {
            Iterable<OutputPort> _outputVariableDeclarationPorts = this.getOutputVariableDeclarationPorts(node);
            for(final OutputPort outputPort : _outputVariableDeclarationPorts) {
              ...
              final CharSequence cDataType = this.dataTypeGenerator.generateDataType(_mscriptGeneratorConfiguration, context, outputDataType, null);
              _builder.newLineIfNotEmpty();
              _builder.append(cDataType, "");
              _builder.append(" ");
              Configuration _configuration_2 = context.getConfiguration();
              String _outputVariableName = GeneratorUtil.getOutputVariableName(_configuration_2, node, outputPort);
  ...
  private Iterable<OutputPort> getOutputVariableDeclarationPorts(final ComponentNode node) {
    Component _component = node.getComponent();
    ...
    Iterable<Output> _filter = IterableExtensions.<Output>filter(_outputs, _function);
    final Function1<Output,EList<OutputPort>> _function_1 = new Function1<Output,EList<OutputPort>>() {
        public EList<OutputPort> apply(final Output o) {
          EList<OutputPort> _ports = o.getPorts();
          return _ports;
        }
      };
    Iterable<EList<OutputPort>> _map = IterableExtensions.<Output, EList<OutputPort>>map(_filter, _function_1);
    Iterable<OutputPort> _flatten = Iterables.<OutputPort>concat(_map);
    final Function1<OutputPort,Boolean> _function_2 = new Function1<OutputPort,Boolean>() {
        public Boolean apply(final OutputPort p) {
          boolean _isConnectedToOutportOnly = InternalGeneratorUtil.isConnectedToOutportOnly(p, node);
          boolean _not = (!_isConnectedToOutportOnly);
          return Boolean.valueOf(_not);
        }
      };
    Iterable<OutputPort> _filter_1 = IterableExtensions.<OutputPort>filter(_flatten, _function_2);
    return _filter_1;
  }
