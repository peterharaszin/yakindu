...
* @SpeedControl_initialize(void)@ function, which must be called only once, and the
* @SpeedControl_execute(const SpeedControl_Input *input, SpeedControl_Output *output)@ function, which must be invoked periodically.
...
bc.. void speedControlTask(void) {
	SpeedControl_Input input;
	SpeedControl_Output output;
	...
	for (;;) {
		...
		SpeedControl_execute(&input, &output);
