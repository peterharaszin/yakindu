...
public class StructLiteralDeclaration extends AbstractCodeFragment {
  ...
  @Inject
  public StructLiteralDeclaration(@Assisted final ComputationModel computationModel, @Assisted final StructValue value) {
  ...
  public void initialize(final ICodeFragmentContext context, final IProgressMonitor monitor) {
    ...
    StructType _dataType = this.structValue.getDataType();
    MachineStructType _create = MachineDataTypes.create(this.computationModel, _dataType);
    StructTypeDeclaration _structTypeDeclaration = new StructTypeDeclaration(this.computationModel, _create);
    StructTypeDeclaration structTypeDeclaration = _structTypeDeclaration;
    StructTypeDeclaration _addCodeFragment = codeFragmentCollector.<StructTypeDeclaration>addCodeFragment(structTypeDeclaration, monitor);
    structTypeDeclaration = _addCodeFragment;
    String _name = structTypeDeclaration.getName();
    this.typeName = _name;
    IGlobalNameProvider _globalNameProvider = context.getGlobalNameProvider();
    String _newGlobalName = _globalNameProvider.newGlobalName("structure");
    this.name = _newGlobalName;
    CharSequence _generateInitializer = this.literalGenerator.generateInitializer(this.computationModel, codeFragmentCollector, this.structValue);
