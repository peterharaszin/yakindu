	...
	private void launch(IPath path, String mode) {
		...
		if (launchConfigurations == null) {
			try {
				...
				String name = launchManager.generateLaunchConfigurationName(path.removeFileExtension().lastSegment());
				ILaunchConfigurationWorkingCopy launchConfiguration = launchConfigurationType.newInstance(null, name);
				launchConfiguration.setAttribute(SimulationLaunchConfigurationDelegate.ATTRIBUTE__FRAGMENT_URI, path.toString());
		...
		try {
			...
			if (launchConfigurationType != null) {
				...
				for (ILaunchConfiguration launchConfiguration : launchManager.getLaunchConfigurations(launchConfigurationType)) {
					String blockDiagramPath = launchConfiguration.getAttribute(SimulationLaunchConfigurationDelegate.ATTRIBUTE__FRAGMENT_URI, "");
