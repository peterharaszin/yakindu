 ...
 *
 */
package org.mda4e.statemachine.contribution.drawing;
...
public class GraphicsUtil {
	...
	/**
	 * This function fills a rounded rectangle with a vertical gradient. This implementation does not use the 
	...
	public static void fillVerticalGradientRoundedRectangle(Graphics graphics, Rectangle bounds, Dimension corner, Color c1, Color c2) {
		...
		p.addArc(
				bounds.x, 
				bounds.y+bounds.height-corner.height-1, 
				corner.width, corner.height, 
				180, 90);
		p.addArc(
				bounds.x+bounds.width-corner.width-1, 
				bounds.y+bounds.height-corner.height-1, 
				corner.width, corner.height, 
				270, 90);
		graphics.fillPath(p);
	 ...
	 * @param baseColor
	 * @param mixinColor
	 * @param ratio a value from 0 to 255 that defines the mix ratio. Using 0 will return the base color and 255 the mixin color.
	 * @return
	 */
	public static Color mixColor(Color baseColor, Color mixinColor, int ratio) {
		return new Color(baseColor.getDevice(), 
				baseColor.getRed() + (mixinColor.getRed() - baseColor.getRed())*ratio/255,
				baseColor.getGreen() + (mixinColor.getGreen() - baseColor.getGreen())*ratio/255,
