...
public class GridLayout extends AbstractLayout
{
    ...
    {        
        ...
        Dimension border = getBorderPreferredSize(container);
        
        int width = wHint;
        if (width >= 0)
        {
            width -= insets.getWidth() - border.width;
            if (width < 0)
            {
                width = 0;
            }
        ...
        if (height >= 0)
        {
            height -= insets.getHeight() - border.height;
            if (height < 0)
            {
                height = 0;
            }
        ...
        Dimension size = layout(container, false, 0, 0, width, height, /* flushCache */ true);
        
        size = new Dimension(size.width + insets.getWidth(), size.height + insets.getHeight()).union(border);
        ...
        /*
         * Piece of code copied from XYLayout to take in account the size of the insets and the
         * size of the border
         */
        return size;
    }
    ...
    Dimension layout(IFigure container, boolean move, int x, int y, int width, int height, boolean flushCache)
    {
            ...
            for (int i = 0; i < rowCount; i++)
            {
                ...
                if (data != null)
                {
                    ...
                    if (hSpan > 1)
                    {
                        ...
                        if (data.widthHint != SWT.DEFAULT || !data.grabExcessHorizontalSpace)
                        {
                            w = data.cacheWidth + data.horizontalIndent - spanMinWidth - (hSpan - 1)
                                    * horizontalSpacing;
                            if (w > 0)
                            {
                                if (spanExpandCount == 0)
        ...
        if (makeColumnsEqualWidth)
        {
            ...
            columnWidth = width == SWT.DEFAULT || expandCount == 0 ? columnWidth : Math.max(minColumnWidth,
                            ...
                            if (data != null)
                            {
                                ...
                                if (hSpan > 1)
                                {
                                    if (data.widthHint != SWT.DEFAULT || !data.grabExcessHorizontalSpace)
                                    {
                                        ...
                                        int w = data.cacheWidth + data.horizontalIndent - spanWidth - (hSpan - 1)
                                                * horizontalSpacing;
                                        if (w > 0)
                                        {
                                            if (spanExpandCount == 0)
