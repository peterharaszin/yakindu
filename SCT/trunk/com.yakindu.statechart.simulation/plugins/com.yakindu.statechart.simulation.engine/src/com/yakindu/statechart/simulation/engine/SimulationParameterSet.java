...
public class SimulationParameterSet extends AbstractStatemachineParameterSet {
	...
	public void setEngineSpecificParameters(
		...
		try {
			String value = configuration.getAttribute(TIME_SCALING, TIME_SCALING_DEFAULT);
			value = (value.trim().length() == 0) ? TIME_SCALING_DEFAULT :value;
			timeScaling = Double.parseDouble(value);
		...
		try {
			String value = configuration.getAttribute(SCHEDULER_INTERVAL, SCHEDULER_INTERVAL_DEFAULT);
			value = (value.trim().length() == 0) ? SCHEDULER_INTERVAL_DEFAULT :value;
			schedulerInterval = Long.parseLong(value);
