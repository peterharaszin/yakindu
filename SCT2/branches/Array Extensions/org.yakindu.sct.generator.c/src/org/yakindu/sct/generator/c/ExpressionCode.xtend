...
class ExpressionCode {
	...
	def dispatch CharSequence code(ElementReferenceExpression it, VariableDefinition target) '''«IF arrayAccess»«target.access»«FOR index : arraySelector»[«index.code»]«ENDFOR»«ELSE»«target.access»«ENDIF»'''
	...
	def dispatch CharSequence code(FeatureCall it, VariableDefinition target) '''«IF arrayAccess»«target.access»«FOR index : arraySelector»[«index.code»]«ENDFOR»«ELSE»«target.access»«ENDIF»'''	
