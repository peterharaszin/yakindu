...
class ExpressionCode {
	...
	def dispatch String code(ElementReferenceExpression it) '''
		...
		«ELSEIF arrayAccess»	
			«reference.code»«getSelectorExpression(arraySelector)»
		«ELSE»
			«definition.code»
		«ENDIF»
	'''

	def String getSelectorExpression(EList<Expression> expressions) {
		var exp = ""
		for (expression : expressions){
			exp = exp + '['+ expression.code +']'
		}
		return exp
	}
	...
	def dispatch String code(FeatureCall it) '''
		...
		«ELSEIF arrayAccess»	
			«definition.code»«getSelectorExpression(arraySelector)»
		«ELSE»
