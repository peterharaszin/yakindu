...
class Navigation extends SExecExtensions {
	...
	def String arrayInitExpression(VariableDefinition variable) {
		if (variable.initialValue != null) {
			generateArrayInitialzation(variable)
		}
		else generateArrayInstatiation(variable.type as ArrayType)
	}
	
	def String generateArrayInstatiation(ArrayType type) {
		return 'new ' + type.elementType.toString.toFirstUpper + getSelectorExpression(type.arraySelector)
	}
	
	def String getSelectorExpression(EList<Integer> selector) {
		var exp = ""
		for (index : selector) {
			exp = exp + '['+ index +']'
		}
	...
	def String generateArrayInitialzation(VariableDefinition definition) {
		StextGeneratorUtil.createArrayListString(definition.initialValue as ArrayInitializationExpression)
	}	
