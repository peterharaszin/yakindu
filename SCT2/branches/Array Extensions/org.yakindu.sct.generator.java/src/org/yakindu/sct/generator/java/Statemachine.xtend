...
class Statemachine {
	...
	def private createFieldDeclarations(ExecutionFlow flow, GeneratorEntry entry) '''
		...
		private «(variable.type as ArrayType).elementType.toString.toFirstUpper» «getArrayDescriptor(variable)» «variable.name.asEscapedIdentifier»  = «arrayInitExpression(variable)»;
		«ELSE»
	...
	def String getArrayDescriptor(VariableDefinition varDef) {
		var descriptor = ""
		for (index : (varDef.type as ArrayType).arraySelector) {
