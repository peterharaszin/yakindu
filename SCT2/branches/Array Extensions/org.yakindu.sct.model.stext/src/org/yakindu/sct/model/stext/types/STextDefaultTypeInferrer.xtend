...
class STextDefaultTypeInferrer extends DefaultExpressionsTypeInferrer implements ISTextTypeInferrer {
	...
	def dispatch InferenceResult doInferType(VariableDefinition definition) {
		if (definition.type == null) {
			...
			if (definition.type instanceof ArrayType) {
				varType = new InferenceResult((definition.type as ArrayType).elementType) 
			}
	...
	def dispatch InferenceResult doInferType(ArrayInitializationExpression exp) {
		if (exp.eContainer instanceof VariableDefinition){
		return new InferenceResult (new InferredType(((exp.eContainer as VariableDefinition).type as ArrayType).elementType))	
		}
		return new InferenceResult(new InferredType(voidType))
	}
...
//		throw new UnsupportedOperationException("Unsupported expression type " + e)
	...
	def dispatch InferenceResult doInferType(FeatureCall featureCall) {
	   ...
	   else if (featureCall.arrayAccess && featureCall.feature instanceof VariableDefinition) {
			return new InferenceResult(new InferredType(((featureCall.feature as VariableDefinition).type as ArrayType).elementType))
		}
		
		if (featureCall.feature != null && featureCall.feature instanceof Feature) {
