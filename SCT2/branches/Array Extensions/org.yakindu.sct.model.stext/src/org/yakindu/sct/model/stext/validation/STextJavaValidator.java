...
//					checkInitValueType((ArrayInitializationExpression) initialValue, ((ArrayType)definition.getType()).getElementType(), definition.getName());
	...
	private void checkSelectorInitialization(EList<Integer> arraySelector,	ArrayInitializationExpression initExp, String arrayName, int index) {
		if (initExp.getValues() != null && !initExp.getValues().isEmpty()) {
			if (initExp.getValues().size() > arraySelector.get(index)) {
				error("Initial value for array " +arrayName+ " exceeds the upper bounce of " + arraySelector.get(index) , null);
			}
		}
		else if (initExp.getDim() != null && !initExp.getDim().isEmpty()) {
			if (initExp.getDim().size() > arraySelector.size()) {
				error("Initial value for array " +arrayName+ " exceeds the definied number of dimensions of " + arraySelector.size(), null);
				return;
			}
			for (Expression dim : initExp.getDim()) {
				checkSelectorInitialization(arraySelector,(ArrayInitializationExpression) dim,arrayName,index);
				index = index+1;
			}
		}
	}
	
	private void checkInitValueType(ArrayInitializationExpression initExp, Type type, String arrayName) {
		for (Literal initValueType : initExp.getValues()) {
			InferenceResult result = typeSystem.inferTypeForLiteral(getLiteralValue(initValueType));
			if (result.getType() == null || !result.getType().getType().toString().equals(type.toString())) {
					error("Array " + arrayName + " of type " + type.toString() + " contains invalid values of type " + result.getType().getType().toString(), null);
			}
		}
		for (Expression dim : initExp.getDim()) {
			checkInitValueType((ArrayInitializationExpression)dim, type,arrayName);
		}
	}
	
	private Object getLiteralValue(Literal initValueType) {
		if (initValueType instanceof StringLiteral) {
			return ((StringLiteral) initValueType).getValue();
		} 
		else if (initValueType instanceof BoolLiteral) {
			return ((BoolLiteral) initValueType).isValue();
		} 
		else if (initValueType instanceof IntLiteral) {
			return ((IntLiteral) initValueType).getValue();
		} 
		else if (initValueType instanceof RealLiteral) {
			return ((RealLiteral) initValueType).getValue();
		} 
		return null;
	}
	...
	public void checkFeatureCall(FeatureCall call) {
		...
		else if (call.isArrayAccess() && call.getFeature() instanceof VariableDefinition) {
			VariableDefinition variableDefinition = (VariableDefinition) call.getFeature();
			EList<Integer> arraySelector = ((ArrayType)variableDefinition.getType()).getArraySelector();
			if (call.getArraySelector().size() > arraySelector.size()) {
				error("The variable  " + variableDefinition.getName() + " exceeds the number of dimensions", ExpressionsPackage.Literals.FEATURE_CALL__FEATURE,	INSIGNIFICANT_INDEX, FEATURE_CALL_TO_SCOPE);
			}
			for (Expression expression : call.getArraySelector()) {
				InferenceResult result = typeInferrer.inferType(expression);
				if (result.getType() == null || !typeSystem.isIntegerType(result.getType())) {
					error("Index of array " + variableDefinition.getName() + " must be of type integer", ExpressionsPackage.Literals.FEATURE_CALL__FEATURE, INSIGNIFICANT_INDEX, FEATURE_CALL_TO_SCOPE);
				}
			}
			for (int i = 0; i < arraySelector.size(); i++) {
				Expression handledArraySelector = call.getArraySelector().get(i);
				Integer definedArraySelector = arraySelector.get(i);
				if (handledArraySelector instanceof PrimitiveValueExpression && ((PrimitiveValueExpression) handledArraySelector).getValue() instanceof IntLiteral) {
					if( ((IntLiteral)((PrimitiveValueExpression) handledArraySelector).getValue()).getValue() >= definedArraySelector){
						error("Index of array "+ variableDefinition.getName() +" exceeds the upper bounce of " + definedArraySelector + " for dimension "  + (i+1), ExpressionsPackage.Literals.FEATURE_CALL__FEATURE, INSIGNIFICANT_INDEX, FEATURE_CALL_TO_SCOPE);
					}
				}
			}
		}
	...
	public void checkFeatureCall(ElementReferenceExpression call) {
		...
		else if (call.isArrayAccess() && call.getReference() instanceof VariableDefinition){
			VariableDefinition reference = (VariableDefinition) call.getReference();
			EList<Integer> arraySelector = ((ArrayType)reference.getType()).getArraySelector();
			if (call.getArraySelector().size() > arraySelector.size()){
				error("The variable " + reference.getName() + " exceeds the number of dimensions", ExpressionsPackage.Literals.ELEMENT_REFERENCE_EXPRESSION__REFERENCE, INSIGNIFICANT_INDEX, FEATURE_CALL_TO_SCOPE);
			}
			for (Expression expression : call.getArraySelector()) {
				InferenceResult result = typeInferrer.inferType(expression);
				if (result.getType() == null || !typeSystem.isIntegerType(result.getType())) {
					error("No integer index array access",ExpressionsPackage.Literals.ELEMENT_REFERENCE_EXPRESSION__REFERENCE, INSIGNIFICANT_INDEX,	FEATURE_CALL_TO_SCOPE);
				}
			}
			for (int i = 0; i < arraySelector.size(); i++) {
				Expression handledArraySelector = call.getArraySelector().get(i);
				...
				if (handledArraySelector instanceof PrimitiveValueExpression && ((PrimitiveValueExpression) handledArraySelector).getValue() instanceof IntLiteral) {
					if ( ((IntLiteral)((PrimitiveValueExpression) handledArraySelector).getValue()).getValue() >= definedArraySelector){
						error("Index of array "+ reference.getName() +" exceeds the upper bounce of " + definedArraySelector + " for dimension "  + (i+1),ExpressionsPackage.Literals.ELEMENT_REFERENCE_EXPRESSION__REFERENCE, INSIGNIFICANT_INDEX,	FEATURE_CALL_TO_SCOPE);
					}
				}
			}
		}
