...
public class SimulationView extends ViewPart implements IDebugContextListener,
	...
	//TODO: Use virtual clock for both
	...
	private void handleDebugEvent(DebugEvent debugEvent) {
		...
		case DebugEvent.RESUME:
			pauseTime += (System.currentTimeMillis() - startSuspendTime);
			getClock().resume();
			break;
		}
	}
		...
		public void run() {
			if (!debugTarget.isSuspended()) {
				...
				if (virtualClock.getStartTime() > 0) {
					...
					if (lblRealTime != null && !lblRealTime.isDisposed()) {
						String text = formatDuration(System.currentTimeMillis()
								- pauseTime - virtualClock.getStartTime(),
								PATTERN);
