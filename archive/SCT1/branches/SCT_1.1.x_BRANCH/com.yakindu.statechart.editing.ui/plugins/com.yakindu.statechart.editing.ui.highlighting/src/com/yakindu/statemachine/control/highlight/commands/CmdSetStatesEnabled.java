...
public class CmdSetStatesEnabled extends AbstractTransactionalCommand {
	
	private List <StateEditPart> activeStates;
	private IPreferenceStore store;

	public CmdSetStatesEnabled(TransactionalEditingDomain domain, String label,
	...
	protected CommandResult doExecuteWithResult(IProgressMonitor arg0,
		...
		for (int i=0;i<activeStates.size();i++)
			activeStates.get(i).getPrimaryShape().setBackgroundColor(new Color(null,PreferenceConverter.getColor(store, Constants.P_STATE_ACTIVE_COLOR)));
		return null;
	}
}