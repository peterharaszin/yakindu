...
This test set the state to st_afterTest_State4 and runs sm_afterTest_runCycle(). It is expected, that
at the first run the function returns FALSE to indicate that the state machine has not entered a stable
state. As there is a unconditioned transition to state 1. This state is reached after the first run.
Therefore the variable st is set to 1. The variable waitTime is not touched in any way, as the state 3
is not entered, nor is state 4.

The second call of the function sm_afterTest_runCycle() returns TRUE, as there is no trigger for
a transition.
*/
void test_sm_afterTest_runCycle_inState4_RTC()
{
...
The test expects that state 4 is left to reach state 1 independent of the trigger, that is set.
...
*/
void test_sm_afterTest_runCycle_inState4_trigger1()
{
	AfterTest_Handle handle;
...
Furthermore the variable st is set to 1 and the variable waitTime is not touched.
*/
void test_sm_afterTest_runCycle_inState4_trigger2()
{
	AfterTest_Handle handle;
...
*/
void test_sm_afterTest_runCycle_inState4_key2_uncleared()
{
	AfterTest_Handle handle;
	...
	assert_equals(st_afterTest_State1, handle.state);
	assert_equals(1, afterTest_getVar_st(&handle));
	assert_equals(300, afterTest_getVar_waitTime(&handle));
	assert_equals(FALSE, afterTest_Iface_isTriggerRaised(&handle.ihandle, trigger1));
	assert_equals(FALSE, afterTest_Iface_isTriggerRaised(&handle.ihandle, trigger2));
	assert_equals(FALSE, afterTest_Iface_isTriggerRaised(&handle.ihandle, key1));
...
/**
#TEST: Testing for state machine behavior when key2 is raised and triggers are cleared after a call to runCycle()
...
*/
void test_sm_afterTest_runCycle_inState4_key2_cleared()
{
	AfterTest_Handle handle;
...
*/
void test_sm_afterTest_runCycle_inState4_key1()
{
	AfterTest_Handle handle;
...
Therefore the whole behavior here is very strange here. This test doesn't run to completion.
*/
void test_sm_afterTest_runCycle_inState4_timer1()
{
	AfterTest_Handle handle;
